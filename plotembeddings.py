...
def main():
    ...
    parser.add_argument(
        ...
        metavar="split",
        type=int,
        help="N split for, 5 means 20% will be used for clustering vizualisation.",
        required=True,
    )
    parser.add_argument(
        "--seed", metavar="seed", type=int, help="random seed", required=True
    )
    parser.add_argument(
        "--maxdatapoints",
    ...
    parser.add_argument(
        ...
        "--modelfiles",
        metavar="modelfiles",
        help="the model files of the different similiarity measurement methods.",
        type=lambda s: [item for item in s.split(",")],
        dest="modelfiles",
    )
    ...
    dsl, colmap, stratified_fold_generator = fromDataSetToSKLearn(
        dataset, oldargs.onehot, n_splits=args.split
    )
    ...
    for method in oldmethods:
        ...
        model, embeddingmodel = modelmaker(
            features, targets, oldargs.hiddenlayers, regression=False
        )
        ...
        indexes, modeltargets = plotEmbeddingClusters(
            save_directory,
            embeddingmodel,
            randommodel,
            filename + method,
        ...
        if args.doeval:
            print(
                "evaluating the clustering performance using a square of the same indexes"
            )
            # def eval_dual_ann(model, test_data, test_target, train_data,
            # train_target, batch_size, anynominal=False, colmap=None, gabel=False)
            evalscore = sillouettescore(model, features[indexes], modeltargets)
