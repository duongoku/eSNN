...
from dataset.makeTrainingData import makeGabelTrainingData, makeDualSharedArchData
from models.utils import (
    normalizeBatchSize,
    makeAndCompileNormalModel,
...
def makeDualArch(
    ...
    model, output = makeAndCompileNormalModel(
        Y.shape[1],
        X.shape[1],
        optimizer=optimizer,
    ...
    subtracted = tf.keras.layers.Subtract()(
        [second_last_layer_orig.output, second_last_layer_clone.output]
    )

    dense_1 = tf.keras.layers.Dense(10, activation="relu")(subtracted)
    ...
    model = tf.keras.Model(inputs=[model.input, model_clone.input], outputs=[output])
    embeddingmodel = tf.keras.Model(
        inputs=[model.input], outputs=[second_last_layer_orig]
    )
...
def makeEndToEndDualArch(
    ...
    val_training_data = [
        val_features[:, 0 : X.shape[1]],
        val_features[:, X.shape[1] : 2 * X.shape[1]],
    ]
...
def makeEndToEndDualArchShared(
    o_X,
    o_Y,
    X,
    Y,
    ...
    for networklayer in networklayers:
        ...
        # dl2 = tf.keras.layers.Dense(int(networklayer),
...
def dees_resnet(
    ...
    subbed = tf.keras.layers.Subtract()([t1, t2])
    # concatted = Concatenate()([t1,t2]) #"residual" to carry over the signals lost in the subtraction..?
    o_t = tf.keras.layers.Concatenate()([t1, t2, subbed])
    # o_t = tf.keras.layers.Dense(int(networklayer[0]),activation="relu")()
    for networklayer in networklayers:
        o_t = tf.keras.layers.Dense(int(networklayer), activation="relu")(o_t)
