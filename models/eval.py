...
def eval_normal_ann_l2(
    ...
    for q_instance, q_target_instance in zip(test_pred_vec, test_target):
        ...
        for target_dim in range(0, test_target.shape[1]):
            if not np.rint(closest[target_dim]) == np.rint(
                q_target_instance[target_dim]
            ):
                allSame = False
...
def _eval_dual_ann(
    ...
    for i in range(0, test_data.shape[0]):
        ...
        combinedata[
            (i * train_data.shape[0]) : ((i + 1) * train_data.shape[0]),
            (train_data.shape[1] * 2) : (train_data.shape[1] * 2)
            + test_target.shape[1],
        ] = train_target
        ...
        combinedata[
            ...
            + test_target.shape[1] : (train_data.shape[1] * 2)
            + (2 * test_target.shape[1]),
    ...
    if isinstance(model, tf.keras.Sequential) or gabel:
        ...
        pred_vec = model.predict(
            [
                ...
                combinedata[:, train_data.shape[1] : 2 * train_data.shape[1]],
            ],
            batch_size=batch_size,
        )
    else:
    ...
    combinedata[
        :,
        (train_data.shape[1] * 2)
        + (2 * test_target.shape[1]) : (train_data.shape[1] * 2)
        + (2 * test_target.shape[1])
        + 1,
    ] = pred_vec
    ...
    for i in range(0, test_data.shape[0]):
        ...
        subset = combinedata[
            (i * train_data.shape[0]) : ((i + 1) * train_data.shape[0]),
            0 : (train_data.shape[1] * 2) + (test_target.shape[1] * 2) + 2,
        ]
...
def evalsquare(model, features, targets):
    ...
    for i in range(0, N):
        ...
        print(
            f"{p} % ({i} of {N}) done, time step: {delta} time left {timeleft}",
            end="\r",
        )
...
def sillouettescore(model, features, targets):
    ...
    for i in range(0, N):
        ...
        timeleft = perstep * (N - (i + 1))
        print(
            f"{p:.2f} % ({i} of {N}) done, time step: {delta:.2f} time left {timeleft:.2f}",
            end="\r",
        )
...
def evalSortedsubset(sortedsubset, index, datahape, targetshape):
    ...
    true_target_value = sortedsubset[
        index, (datahape * 2) + targetshape : (datahape * 2) + (2 * targetshape)
    ]
    ...
    if unique_counts[True] > 1:
        reverse = np.argmax(
            subsetsubset[:, (datahape * 2) : (datahape * 2) + targetshape], axis=1
        )
        ...
        if (res2 is not None) and (
            res1[1] == res2[1]
        ):  # the two top classes are equally numbered..
        ...
        this_train_target = sortedsubset[
            index, (datahape * 2) : (datahape * 2) + targetshape
        ]
        return all(np.equal(this_train_target, true_target_value))


def _eval_dual_ann_big(
    model,
    test_data,
    test_target,
    train_data,
    ...
    for i in range(0, test_data.shape[0]):
        ...
        thistest[:, 0 : train_data.shape[1]] = np.tile(
            test_data[i], (train_data.shape[0], 1)
        )
        ...
        if isinstance(model, tf.keras.Sequential) or gabel:
            pred_vec = model.predict(
                thistest[:, 0 : train_data.shape[1] * 2], batch_size=batch_size
            )
        elif isinstance(model, tf.keras.Model):
            pred_vec = model.predict(
                [
                    thistest[:, 0 : train_data.shape[1]],
        ...
        if pred_vec.shape[1] != 1:
            pred_vec = choprascore(pred_vec)
        # finally we add the last column which is the networks evaluation of the similiarity of the pair of datapoints at row i.
        thistest[:, (train_data.shape[1] * 2) : (train_data.shape[1] * 2) + 1] = (
            pred_vec
        )
        # sortecombineddata = combinedata[combinedata[:,combinedata.shape[1]-1].argsort()]
        # now we have the similarities, we need to sort on the network output
        ...
        this_train_target = sortedsubset[
            index, train_data.shape[1] : train_data.shape[1] * 2
        ]
