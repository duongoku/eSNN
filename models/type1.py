...
def sim_def_lin(
    ...
    for test_instance, test_target in zip(test_instances, test_targets):
        ...
        for training_instance, training_target in zip(
            training_instances, training_targets
        ):
            sim = global_sim_def(
                test_instance,
                training_instance,
        ...
        if (most_similar_case_target is not None) and (
            np.rint(test_target) == np.rint(most_similar_case_target)
        ).all():
            res.append(1)
        else:
            res.append(0)
    ...
    if not anynominal:
        ...
        for i in range(0, v1.shape[0]):
            if nominallist[i] is True:
                ...
                res = res + sim_nonlin(
                    v1[i], v2[i], params[i], maximums[i], minimums[i]
                )
    ret = res / len(v1)
    return ret
