...
rundict = {
    "gabel": {
        "modeltrainer": makeGabelArch,
        "eval": eval_gabel_ann,
        "figname": "$gabel$",
    },
    "eSNN": {
        "modeltrainer": esnn,
        "makeModel": make_eSNN_model,
        "eval": eval_dual_ann,
        "figname": "$eSNN$",
    },
    "t3i1": {
        "modeltrainer": makeNormalArch,
        "eval": eval_normal_ann_l2,
        "figname": "$t_{3,1}$",
    },
    "t4i4": {
        "modeltrainer": makeDualArch,
        "eval": eval_dual_ann,
        "figname": "$t_{4,4}$",
    },
    "t4i1": {
        "modeltrainer": makeEndToEndDualArch,
        "eval": eval_dual_ann,
        "figname": "$t_{4,1}$",
    },
    "t4i2": {
        "modeltrainer": makeEndToEndDualArchShared,
        "eval": eval_dual_ann,
        "figname": "$t_{4,2}$",
    },
    "t4i6": {
        "modeltrainer": dees_resnet,
        "eval": eval_dual_ann,
        "figname": "$t_{4,6}$",
    },
...
optimizer_dict = {
    ...
    "adam": {"constructor": tf.keras.optimizers.Adam, "batch_size": None},
    "rmsprop": {"constructor": tf.keras.optimizers.RMSprop, "batch_size": None},
}
...
def getParser():
    parser = argparse.ArgumentParser(
        description="train NN classification" + " model on datasets against CBR!"
    )
    parser.add_argument("--epochs", metavar="epochs", type=int, help="How many epochs")
    parser.add_argument(
        "--evals",
        metavar="evals",
        type=int,
    ...
    parser.add_argument(
        "--gpu", metavar="gpu", type=str, help="which gpu should be used", default=""
    )
    parser.add_argument(
        "--prefix",
        metavar="prefix",
    ...
    parser.add_argument("--test", metavar="test", type=str, help="")
    parser.add_argument(
        "--onehot",
        metavar="True/False",
        type=str2bool,
    ...
    parser.add_argument(
        ...
        metavar="True/False",
        type=str2bool,
        help="Enable detailed cv fold output.",
        default=True,
    )
    parser.add_argument(
        "--printcv",
    ...
    parser.add_argument(
        ...
        metavar="True/False",
        type=str2bool,
        help="Do evaluation after training.",
        default=True,
    )
    parser.add_argument(
        "--batchsize",
    ...
    parser.add_argument(
        ...
        metavar="alphacount",
        type=int,
        help="number of steps when testing alphas",
        default=32,
    )
    parser.add_argument(
        "-alpharange",
    ...
    parser.add_argument(
        "--seed", metavar="seed", type=int, help="Size of batch to use in training"
    )
    parser.add_argument(
        "-ds",
        "--datasets",
    ...
    parser.add_argument(
        ...
        "--c_layers",
        metavar="c_layers",
        help="comma delimited list of hidden layer sizes for " + "c(.,.)",
        type=str,
        nargs="+",
        action="append",
    )
    parser.add_argument(
        ...
        "--g_layers",
        metavar="g_layers",
        help="comma delimited list of hidden layer sizes for " + "g(.)",
        type=str,
        nargs="+",
        action="append",
    )
...
def runalldatasets(
    ...
    for dataset in datasetlist:
            ...
            for key, value in colmap.items():
                ...
                for col in datasetInfo["cols"]:
                    if col["name"] == key and col["class"] == True:
                        isclass = True
                    if value["type"] == "nominal" and not isclass:
                        anynominal = True
        ...
        for train, test in stratified_fold_generator:
            ...
            fold_results, ranlist = rundataset(
                anynominal,
                args,
                callbacks,
                colmap,
...
def iteration(
    ...
    if trainer is not None:
        model, hist, ret_callbacks, embedding_model = trainer(
            o_X=data[test],
            o_Y=target[test],
            X=data[train],
            Y=target[train],
        ...
        if evalcount:
            fold_results[methodstring]["evals"] = maketrainingdata_dict["factorfunc"](
        ...
        if "retain_measure" in callbacks:
            ...
            fold_results[methodstring][
                "training_retain_loss_hist"
            ] = training_retain_losses
    ...
    if printcvresults:
        printCVSummary(dataset, fold_results, methodstring, n, args, fold_number)
...
def printCVSummary(dataset, CV_result, method, n, args, fold_number):
    ...
    if method not in non_trainable:
        ...
        print(
            f"dataset {dataset} method {method} " f"avg_retrieve_loss: {retain_loss} "
        )
