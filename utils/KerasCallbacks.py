...
class RetainCallBack(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        res = self.eval_func(
            self.model,
            self.gvalidation_data,
            self.gvalidation_target,
            self.gtraining_data,
            self.gtraining_target,
            self.batch_size,
        )
...
class Overfitting_callback(tf.keras.callbacks.Callback):
    ...
    def __init__(
        ...
        if mode not in ["auto", "min", "max"]:
            warnings.warn(
                "Overfitting mode %s is unknown, " "fallback to auto mode." % mode,
                RuntimeWarning,
            )
            mode = "auto"
    ...
    def on_epoch_end(self, epoch, logs=None):
        # output_layer = self.model.get_layer("dense_5")
        # multi_output_model = tf.keras.models.Model(
        #     inputs=self.model.inputs, outputs=output_layer.output
        # )
        # o1, o2 = multi_output_model.predict([input_data1, input_data2])
    ...
    def on_train_end(self, logs=None):
        if self.stopped_epoch > 0 and self.verbose > 0:
            print(
                "Epoch %05d: Model overfit. Training stopped."
                % (self.stopped_epoch + 1)
            )


class MyEarlyStop(tf.keras.callbacks.EarlyStopping):
    def __init__(
        self,
        validation_data,
        validation_target,
...
class GabelStop(tf.keras.callbacks.Callback):
    ...
    def on_train_end(self, logs=None):
        if self.stopped_epoch > 0:
            print(
                "Epoch %05d: Gabel reached. Training stopped at."
                % (self.stopped_epoch + 1)
            )
            print(
                f"Got retain loss {self.last_retain_loss} vs gabel {sim_nn_results[self.dataset]}"
            )


class GabelOverfit(tf.keras.callbacks.Callback):
...
class GabelElitism(tf.keras.callbacks.Callback):
