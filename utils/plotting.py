...
def plotTraining(folded_results, method, loss, save_directory):
    ...
    datadict = {
        "epochs": np.arange(
            0, len(folded_results[0][method]["history"]["loss"])
        ).tolist()
    }
    collist = []
...
def plotResults2(datasets, results, save_directory, k_folds):
    ...
    sns_plot = sns.lineplot(
        x="dataset", y="value", style="method", hue="method", data=df
    )
...
def plotResults3(datasets, results, save_directory, k_folds):
    ...
    fig, ax = plt.subplots()
    sns.set(font_scale=0.4)
    sns_plot = sns.lineplot(x="dataset", y="value", hue="method", data=sorted_df)
...
def plotTrainings(datasets, results, save_directory, k_folds):
    ...
    for dataset in datasets:
        ...
        sns.set(font_scale=0.4)
        sns_plot = sns.lineplot(
            x="epoch", y="value", style="method", hue="method", data=df
        )
        ax.set(xlabel="dataset", ylabel="retrieval loss")
        fig = sns_plot.get_figure()
...
def plotNTrainings(
            ...
            for foldnumber in dataset_result.keys():
                ...
                for method in fold:
                    ...
                    for epochvalue in training_loss:
                        epochnum += 1
                        thisdict = {
                            xkey: epochnum * evalFunc(datasetsize),
                            "method": methodname,
                            "fold": fold_counter,
                            "value": epochvalue,
                            "n": i,
    ...
    lastdf = lastdf.drop_duplicates(
        subset=["method", xkey, "n", "mean_value"], keep="first"
    )

    # sns.set(font_scale=0.7)
    setLateXFonts()
    with sns.plotting_context(
        "poster", font_scale=font_scale, rc={"lines.linewidth": 1}
    ):
        fig, ax = plt.subplots()
        if hue_order == None:
            sns_plot = sns.lineplot(
                x=xkey, y="mean_value", style="method", hue="method", data=lastdf
            )
        else:
            sns_plot = sns.lineplot(
                x=xkey, y="mean_value", hue="method", hue_order=hue_order, data=lastdf
            )
...
def create_tpr_fpr(path):
    ...
    for t in np.arange(0.0, 2.02, 0.02):
        ...
        false_positive_mx = np.logical_and(
            predict_is_same, np.logical_not(same_fish_matrix)
        )
...
def plotEmbeddingClusters(
    ...
    maxnum = len(classindexes) if len(classindexes) < maxdatapoints else maxdatapoints
    ...
    with sns.plotting_context(
        ...
        if pdf == True:
            sns_plot.savefig(
                f"{save_directory}/{filename}-pca-random.pdf", bbox_inches="tight"
            )
        else:
            sns_plot.savefig(
                f"{save_directory}/{filename}-pca-random.eps", bbox_inches="tight"
            )
        plt.clf()
    ...
    with sns.plotting_context(
        ...
        if pdf == True:
            sns_plot.savefig(
                f"{save_directory}/{filename}-tsne-random.pdf", bbox_inches="tight"
            )
        else:
            sns_plot.savefig(
                f"{save_directory}/{filename}-tsne-random.eps", bbox_inches="tight"
            )
        plt.clf()
    ...
    with sns.plotting_context(
        ...
        if pdf == True:
            sns_plot.savefig(
                f"{save_directory}/{filename}-pca-trained.pdf", bbox_inches="tight"
            )
        else:
            sns_plot.savefig(
                f"{save_directory}/{filename}-pca-trained.eps", bbox_inches="tight"
            )
        plt.clf()
    ...
    with sns.plotting_context(
        ...
        if pdf == True:
            sns_plot.savefig(
                f"{save_directory}/{filename}-tsne-trained.pdf", bbox_inches="tight"
            )
        else:
            sns_plot.savefig(
                f"{save_directory}/{filename}-tsne-trained.eps", bbox_inches="tight"
            )
        plt.clf()
    return classindexes, modeltargets


def plotNRealTrainings(
    nresults,
    save_directory,
    filename,
    font_scale,
    ...
    with sns.plotting_context(
        ...
        fig, ax = plt.subplots()
        if hue_order == None:
            sns_plot = sns.lineplot(x=xkey, y="mean_value", hue="method", data=lastdf)
        else:
            sns_plot = sns.lineplot(
                x=xkey, y="mean_value", hue="method", hue_order=hue_order, data=lastdf
            )
...
def plotNAlphaResults(
    nresults,
    save_directory,
    font_scale=0.8,
    file_name="all_methods_loss.pdf",
    drawdev=False,
    removeoptimizer=False,
    pdf=True,
):
    ...
    with sns.plotting_context(
        ...
        fig, ax = plt.subplots()
        if drawdev:
            sns_plot = sns.lineplot(x="method", y="value", data=lastdf, sort=False)
        else:
            sns_plot = sns.lineplot(
                x="method", y="method_mean", data=lastdf, sort=False
            )
...
def plotAlphaResults(datasets, results, save_directory, k_folds):
    ...
    sns.set(font_scale=0.4)
    sns_plot = sns.lineplot(
        x="method", y="dataset_mean", style="dataset", hue="dataset", data=sorted_df
    )
    ax.set(xlabel="dataset", ylabel="retrieval loss")
    fig = sns_plot.get_figure()
    fig.savefig(f"{save_directory}/all_methods_loss.pdf")
...
def nJsonToDF(nresults, k_folds, removeoptimizer=False):
    ...
    for i in range(0, len(nresults)):
        ...
        if lastdf == None:
            ...
            lastdf = pd.concat((lastdf, df), ignore_index=True)
    return lastdf.drop_duplicates(subset=["dataset", "method", "n", "value"]).drop(
        columns=["fold"]
    )
...
def plotDf(
    ...
    with sns.plotting_context(
        ...
        rc={"lines.linewidth": font_scale, "grid.linewidth": font_scale},
    ):
        fig, ax = plt.subplots()
        ...
        if hue_order == None:
            sns_plot = sns.lineplot(
                x="dataset", y=_y, hue="method", data=df, sort=False
            )
        else:
            sns_plot = sns.lineplot(
                x="dataset",
...
def generateData(desired_mean, desired_std_dev, num_samples):
    ...
    print(
        "Initial samples stats   : mean = {:.4f} stdv = {:.4f}".format(
    ...
    print(
        "True zero samples stats : mean = {:.4f} stdv = {:.4f}".format(
            zero_mean_mean, zero_mean_std
        )
    )
    ...
    print(
        "Scaled samples stats    : mean = {:.4f} stdv = {:.4f}".format(
    ...
    print(
        "Final samples stats     : mean = {:.4f} stdv = {:.4f}".format(
...
def plotJson(
    resultfile, k_folds, savedir, filename, add_gabel=False, drawdev=False, plot_loc=1
):
...
def plotNJson(
    ...
    df["sort_method_value"] = df.apply(
        lambda row: getCorrespondingMethodValue(row, df, sortmethodname), axis=1
    )
    if dataset_order == None:
    ...
    plotDf(sorted_df, savedir, filename, drawdev, font_scale, hue_order, legend_loc)
