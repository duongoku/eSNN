...
def makeGabelTrainingData(features, targets, isregression, distance=False):
    ...
    combineddata = np.zeros(
        (features.shape[0] ** 2, 2 * features.shape[1] + ((targets.shape[1] * 2) + 1))
    )
    ...
    if isregression:
        for i in range(features.shape[0]):
            ...
            combineddata[
                i * features.shape[0] : (i * features.shape[0]) + features.shape[0],
                0 : features.shape[1],
            ] = tile
            combineddata[
                i * features.shape[0] : (i * features.shape[0]) + features.shape[0],
                features.shape[1] : (features.shape[1] * 2),
            ] = features
        ...
        for i in range(features.shape[0]):
            ...
            combineddata[
                (i * features.shape[0]) : ((i + 1) * features.shape[0]),
                (features.shape[1] * 2) : (features.shape[1] * 2) + targets.shape[1],
            ] = targets

            combineddata[
                (i * features.shape[0]) : ((i + 1) * features.shape[0]),
                (features.shape[1] * 2)
                + targets.shape[1] : (features.shape[1] * 2)
                + (targets.shape[1] * 2),
            ] = np.tile(targets[i], (features.shape[0], 1))
            for i2 in range(targets.shape[0]):
                counter = 0
    ...
    combineddata[
        :,
        (features.shape[1] * 2)
        + (targets.shape[1] * 2) : (features.shape[1] * 2)
        + (targets.shape[1] * 2)
        + 1,
    ] = targetout

    return (
        combineddata[:, 0 : features.shape[1] * 2],
        combineddata[
            :,
            (features.shape[1] * 2)
            + (targets.shape[1] * 2) : (features.shape[1] * 2)
...
def makeDualSharedArchData(features, targets, isregression, distance=False):
    ...
    if isregression:
        for i in range(features.shape[0]):
            ...
            for i2 in range(targets.shape[0]):
                targetout[(i * features.shape[0]) + i2, 0] = 1.0 - eucdistance(
                    targets[i], targets[i2]
                )
    else:
        for i in range(features.shape[0]):
            tile = np.tile(features[i], (features.shape[0], 1))
...
def makeSmartNData(features, targets, isregression, distance=False):
    ...
    for i in range(0, targets.shape[0]):
        if positive:
            ...
            newdata, targetind = getSameClass(
                features, nonbinarized[i], digit_indices, num_classes, classcounter
            )
            ...
            # class (negative pair)
            newdata, targetind = getOtherClass(
                features, nonbinarized[i], digit_indices, num_classes, classcounter
            )
            combineddata[i, features.shape[1] : features.shape[1] * 2] = newdata
            if distance:
    ...
    return combineddata, sim_target, targets.copy(), targets2
...
def makeSemiBig(features, targets, isregressive, distance=False):
    ...
    for i in range(0, features.shape[0]):
        ...
        combineddata[ibase : ibase + comparedto, : features.shape[1]] = np.tile(
            features[i], (comparedto, 1)
        )
        targets1[ibase : ibase + comparedto, :] = np.tile(targets[i], (comparedto, 1))
        for i2 in range(counter, features.shape[0]):
            combineddata[
                ibase + (i2 - counter), features.shape[1] : 2 * features.shape[1]
            ] = features[i2]
        ...
        counter += 1
        ibase += comparedto
    return combineddata, sim_target, targets1, targets2
