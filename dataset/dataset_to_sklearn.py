...
from dataset.mapper import DataFrameMapper
import math
...
class SKLearnDataset:
    ...
    def containsNonNumericalCols(self):
        for col in self.dataset.datasetInfo["cols"]:
            if col["type"] == "nominal" or col["type"] == "ordinal":
                return True
        return False
...
def makeembeddings(categorical_vars, df):
    ...
    for categorical_var in categorical_vars:
        ...
        model.add(tf.keras.layers.Embedding(vocab, embedding_size, input_length=1))
        model.add(tf.keras.layers.Reshape(target_shape=(embedding_size,)))
        models.append(model)
...
def adapt(df, featuremapper, targetmapper, featurecols, targetcols, datadict, colmap):
...
def fromDataSetToSKLearn(dataset, multilabelbin=True, n_splits=None):
    ...
    if "getInfo" in datasetInfo:
        ...
        target_names, isregression, featurecols, targetcols = getDatasetInfo_dyn(
            df, datasetInfo
        )
    else:
        (
    ...
    if n_splits is not None:
        ...
        stratified_fold_generator = kfold.split(
    ...
    if "adapt" in datasetInfo:
        ...
        features, targets, colnames = adapt(
            df,
            featuremapper,
            targetmapper,
            featurecols,
