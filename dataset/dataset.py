...
def mnist_adapt(df, datasetInfo):
    ...
    targetcols = [
        i for i in range(features.shape[1], features.shape[1] + targets.shape[1])
    ]
    featurecols = [i for i in range(0, features.shape[1])]
...
def mnist_makedf(url, datasetInfo):
    ...
    path = tf.keras.utils.get_file("mnist.npz", origin=url)
    ...
    all_test = np.concatenate(
        (flattentest, y_test.reshape((y_test.shape[0], 1))), axis=1
    )
...
datamap = {
    "soybean-small": {
        ...
        "num_classes": 1,
        "cols": [
            {"name": "date", "type": "nominal", "class": False},
    ...
    "car": {
        ...
        "num_classes": 1,
        "cols": [
            {"name": "bying", "type": "nominal", "class": False},
    ...
    "cmc": {
        ...
        "num_classes": 1,
        "cols": [
            {"name": "wife_age", "type": np.int32, "class": False},
    ...
    "hay": {
        ...
        "num_classes": 1,
        "cols": [
            {"name": "name", "type": "skip", "class": False},
    ...
    "ttt": {
        ...
        "cols": [
            {"name": "top_left", "type": "nominal", "class": False},
    ...
    "who": {
        ...
        ],  # It seems the target variable == "region" or "channel"
    },
    "bupa": {
    ...
    "servo": {
        ...
        "num_classes": 1,
        "cols": [
            {"name": "motor", "type": "nominal", "class": False},
...
def safeGetDF(url):
    ...
    if ".xls" in url:
        ...
        return BytesIO(text)
        # out.write(text)
        # out.write(str(text,encoding="ISO-8859-1"))
...
def convert(inp):
    if inp == "nominal" or inp == "ordinal" or inp:
        return "str"
    elif inp == "binary":
        return np.int32
    else:
...
class Dataset:
    ...
    def loadDataset(self, key):
        ...
        colnames = [c["name"] for c in self.datasetInfo["cols"] if c["type"] != "skip"]
        dtypes = [c["type"] for c in self.datasetInfo["cols"] if c["type"] != "skip"]
